...
/// - X 掃引は坂セルをソリッド扱いしない。さらに足元中心セルが坂のときは
///   **足元の行を側面判定から除外**する（坂を登り切って頂上の平地セルへ
///   乗り移る際、平地セルの角で引っかからないため）。副作用として、足元が
///   坂セル内にある間は「高さ 1 セルだけの障害物」を側面ですり抜けうる
///   （通常のマップ構成では起きない。壁は複数セル or 坂に隣接しない前提）。
