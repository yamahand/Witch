...
public:
    ...
    /// スポーン反映時に呼ばれる。サブクラスはここに初期化処理を書く。
    /// 実行タイミングは Spawn の場所で変わる: OnEnter 中の Spawn では Spawn<T>() の
    /// 呼び出し中に即時実行、更新中の Spawn では次の生成反映ステージで実行される。
    /// どちらでも同じ結果になるよう、初期化は **コンストラクタ引数と自分自身で完結**
    /// させること（Spawn 後の外部からの設定に依存しない = 自己完結契約）。
    /// シーンアクセス（GetScene() / Id() / Scene::Find 等）はコンストラクタでは
    /// 不可（scene_/id_ 未設定）で、ここから使える。
