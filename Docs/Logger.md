...
- TextBuffer はダブルバッファ、LogRecord 配列と分離。容量は `Logger::Config` で可変
  （要件上の上限は 1 面 100MB / 10 万件を想定するが、毎フレーム Flush される前提では
  1 フレーム分しか溜まらないため**実装の既定は 8MB×2 面 / 1 万件**と控えめにしてある。
  逼迫時は Log() 内から強制 Flush されるため既定でも溢れない）
...
Engine::Run のフレーム先頭: logger->SetFrameNumber(time->FrameCount()+1)
  （このフレームで発生する全ログ ── ApplyPendingSceneChange 内も含む ── を同一番号にする）
Engine::Run のフレーム末: logger->Flush()
