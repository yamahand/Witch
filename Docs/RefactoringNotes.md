...
- **マクロ付け忘れの検出**: `WITCH_COMPONENT` を付け忘れると `T::StaticTypeId()` が基底 ID に
  フォールバックし、無関係な型に誤マッチして不正な `static_cast`（未定義動作）になる。
  これを防ぐため `GetComponent<T>` に `static_assert(kHasComponentTypeId<T>)` を置き、
  `&T::StaticTypeId != &Component::StaticTypeId`（定数式）で付け忘れをコンパイル時に弾く。
