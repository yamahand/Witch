...
**現状: M7 物理・衝突完了（M7-a: 2026-07-11 = AABB タイル衝突 + CollisionComponent + CollisionWorld + プレイヤー移動・ジャンプ、M7-b: 2026-07-13 = 45° 床坂 + 衝突デバッグ表示 "Show Collision"）。残は LDtk 側の坂値定義（マップ制作時）と手触り定数の確定。次の実装対象は M8 オーディオ。**
...
| 3  | スプライト上限超過が黙って捨てられる | ✅ 済（16384 + タイルマップ側カリング, 2026-07-10） | — |
...
| 11 | テストが 0 件 | ✅ 済（Catch2 基盤 + 固定ステップ + AABB 衝突 / CollisionWorld のテスト, 2026-07-11） | — |
...
### 3. スプライト上限 1024 超過 — ✅ 対応済み（2026-07-10、M6 タイルマップと同時）
[D3D12Renderer.h](../Engine/Source/Rhi/D3D12/D3D12Renderer.h) —
...
- ✅ **上限を 16384 に引き上げ**（頂点バッファは 2 MiB / バックバッファで無害）。
  あわせて [TilemapComponent](../Engine/Source/Graphics2D/TilemapComponent.cpp) が
  カメラ可視矩形でカリングして提出する（レイヤー AABB の早期リジェクト +
  タイルごとの矩形交差）。超過警告は残してあるので、足りなくなれば観測できる。
- ⬜ 大幅ズームアウト（低倍率で 8px タイル全面）が恒常化して 16384 でも
  足りなくなったら、タイルマップのインスタンシング描画（専用 RHI パス）を検討。
...
問題だった（描画上の遅延は固定タイムステップ導入時の FrameUpdate 先頭 flush で解消済み）。

採用した対策は「OnEnter 後に flush」ではなく **OnEnter 中の Spawn の即時反映**
（Unity の Instantiate + Awake 同期実行と同型）。flush 案では
「LoadLevel → 直後に Find で配線」という OnEnter **内部**のユースケースに間に合わないため。

- [Scene.h](../Engine/Include/WitchEngine/Scene/Scene.h) に非仮想 `Enter()` / `Exit()`（NVI）
  を追加し、`OnEnter` / `OnExit` は protected に。Engine は `Enter()` を呼ぶ。
- `Enter()` が `inEnter_` を立て、その間の `Spawn<T>()` は保留リストを経由せず
  `CommitSpawn()`（OnSpawn → scheduler 登録 → objects_。flush と同一処理の単一ソース）
  で即時反映。戻り時に `Find` が通り OnSpawn 完了済み。更新中の Spawn は従来どおり保留。
- 契約（[GameObject.h](../Engine/Include/WitchEngine/Scene/GameObject.h) の OnSpawn に明文化）:
  OnSpawn は**コンストラクタ引数と自分自身で完結**させる（Spawn 後の外部設定に依存しない）。
  `Spawn<T>(args...)` はコンストラクタへ完全転送されるので、必要な情報は引数で渡す。
  コンストラクタでは `GetScene()` / `Id()` は未設定（シーンアクセスは OnSpawn から）。
- テスト: SceneTests.cpp「OnEnter 中の即時 Spawn」セクション
  （即時反映・両モードでの OnSpawn 等価性・入れ子 Spawn・反映後 AddComponent の登録経路）。

### 9. クリア色が GameLoop にハードコード — ✅ 対応済み（2026-07-10、M6 と同時）
[Scene.h](../Engine/Include/WitchEngine/Scene/Scene.h) に `clearColor_` +
`SetClearColor()` / `ClearColor()` を追加（既定は従来のコーンフラワーブルー）。
[GameLoop.cpp](../Engine/Source/Core/GameLoop.cpp) はシーンがあれば
`currentScene->ClearColor()` で Clear する（シーン無しの間のみ従来定数）。
`Scene::LoadLevel` がレベルの背景色（LDtk の `__bgColor`）で上書きするため、
マップごとの背景色が出る。
...
- **構成**: `Engine/Tests/`（WitchEngineTests 実行ファイル）。`WITCH_BUILD_TESTS`
  オプション（既定 OFF、テスト専用の debug-tests / ci-* プリセットで ON。
  日常開発用の debug / debug-profiling と配布用 release はテスト非依存）。
  ローカル実行は `cmake --build --preset debug-tests` → `ctest --preset test-debug`。
...
- ✅ **済（2026-07-11、M7-a）**: AABB 衝突解決のテストを追加
  （TileCollisionTests = 純関数の掃引・押し戻し・トンネリング防止・境界接触、
  CollisionComponentTests = 速度積分・フェーズ順・着地・layer/mask・
  コールバック契約・LoadLevel リロード時のキャッシュ引き直し）。
