...
| 11 | テストが 0 件 | △ 基盤導入済み（Catch2 + ctest, 2026-07-09） | 固定ステップのテストは §3 実装時 |
...
- **RHI 抽象**: `IRenderer::SetCamera(scale, offsetX, offsetY)` を追加
  （`screen = world * scale + offset`）。GameLoop が Scene::Update 後
...
- ⬜ **残**: §3 固定タイムステップ（アキュムレータ）のテストは実装時に追加。
  AABB 衝突解決（M7）もテスト対象に加える。
