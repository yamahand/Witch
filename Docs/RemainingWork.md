...
- [x] **AABB vs タイルマップの衝突解決**: 移動 → 軸ごとに押し戻し（X 掃引 → Y 掃引の
      ...
      読み書きのみで制御する。
- [x] **坂タイル対応**（M7-b、2026-07-13）: 45° 床坂。IntGrid 値 **2 = 右上がり '/'、
      3 = 左上がり '\'**（`ShapeFromValue` に集約。天井坂は消費者が出てから）。
      接地は AABB 下辺の**中心点方式**（洞窟物語と同型）: Y パスで坂表面へ
      スナップ（登坂の押し上げ含む）、X パスは坂を非ソリッド扱い + 足元セルが
      坂のとき足元行を側面判定から除外（頂上の平地への乗り移りで引っかからない）、
      中心点のソリッド食い込み（< 1 セル）は上へ押し出し。
      `MoveAabb(…, snapToGround)` で直下 1 セル以内の地面へ吸着
      （下り坂・小段差降りの浮き防止。CollisionComponent が前ステップ接地中のみ渡す）。
      **⬜ 残（コンテンツ作業）**: LDtk エディタ側で Sample1_1.ldtk の IntGrid 値
      定義に 2/3 を追加してマップに坂を置く（エンジン・テストは対応済み）。
...
- [x] **IAudio サービス**: `Services` に `audio` メンバを追加（CLAUDE.md の仕様には
      ...
      → `Audio/IAudio.h` + `Engine/Source/Audio/MiniaudioAudio.cpp`（1 TU に隔離）。
      Ogg Vorbis は stb_vorbis（vcpkg stb）を同 TU に組み込んで有効化。
      デバイス初期化失敗は非致命（audio スロット null のまま無音続行）。
- [x] **SE 再生**: 多重再生（同じ SE の重ね掛け）、音量指定。
      → ボイスごとに独立デコーダ（エンコード済みバイト列から逐次デコード）。
      終了ボイスは GameLoop の毎フレーム `IAudio::Update()` で回収。
- [x] **BGM 再生**: ストリーミング + **ループポイント指定**
      （洞窟物語の BGM はイントロ付きループ。先頭からのループでは再現できない）。
      → メモリ上のエンコード済みデータを逐次デコード（= ストリーミング相当）。
      `BgmParams::loopBeginSeconds` で末尾 → 指定位置へ折り返す。
- [x] ResourceManager に音声アセットのロード / キャッシュを追加。
      → `LoadAudio()`。再生中ボイスが shared_ptr を保持するため UnloadAll 中も安全。
- [ ] （必要になったら）デコード済み PCM キャッシュ。現状はアセットが小さく
      再生ごとの逐次デコードで足りるため見送り（プロファイルで効いたら追加）。
...
| M7 | ✅ 完了（M7-a: 2026-07-11 / M7-b 坂 + 衝突デバッグ表示: 2026-07-13）。**残: LDtk 側の坂値定義（マップ制作時）+ 手触り定数の確定** | 5 |
| ~~M8~~ | ✅ 完了 (2026-07-17): オーディオ（SE + ループ BGM）。**残: デコード済み PCM キャッシュ（必要になったら）** | 6 |
