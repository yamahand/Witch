...
- [ ] **仮想解像度（固定視界）カメラ**: 「画面に見えるワールド範囲」をウィンドウ
      サイズと切り離して固定する。現状はカメラのビューポート＝ウィンドウ実サイズ
      なので、大きいウィンドウの人ほど遠くまで見える（レベルデザインが成立しない）。
      基準視界（例: 1920x1080 相当）を決め、カメラがウィンドウへ一様スケールで写像し、
      アスペクト違いはレターボックスで吸収する。描画はネイティブ解像度のまま行う。
      ※ レトロ風の低解像度レンダーターゲット + 整数倍拡大は**採用しない**
      （見た目をレトロにする意図はない。アート方針は未定・保留）。
      基準視界の具体値と「1 タイル何 px 相当か」はアート方針決定時に確定する。
